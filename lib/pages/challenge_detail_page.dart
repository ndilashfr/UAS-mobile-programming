...
Future<void> _checkRankAndNotify() async {
    ...
    try {
      ...
      final snapshot = await FirebaseFirestore.instance
          ...
          .where('challengeId', isEqualTo: widget.challengeId) // Filter Challenge ID
      ...
      if (myRank > 0) {
        String message = "Progres mantap! Kamu sekarang ada di Peringkat #$myRank untuk challenge ini.";
        
        // Pesan spesial kalau masuk Top 3
        if (myRank == 1) {
           message = "RAJA CHALLENGE! 👑 Kamu sekarang memimpin di Peringkat #1!";
        } else if (myRank <= 3) {
 ...
 Future<void> _joinChallenge() async {
    ...
    try {
      ...
      final challengeDocRef = FirebaseFirestore.instance.collection('challenges').doc(widget.challengeId);
      final challengeSnapshot = await challengeDocRef.get();
      
      if (!challengeSnapshot.exists) throw Exception("Challenge tidak ditemukan!");
      ...
      final participantDocRef = FirebaseFirestore.instance.collection('participants').doc(participantDocId);
  ...
  Future<void> _updateProgress() async {
    setState(() { _isLoading = true; });
    if (_user == null) { /* ... handle error ... */ setState(() { _isLoading = false; }); return; }
    ...
    try {
      ...
      // --- PERUBAHAN KUNCI DIMULAI DARI SINI ---

      // 1. Ambil data Challenge (hanya untuk durasi)
      final challengeDoc = await challengeDocRef.get();
      if (!challengeDoc.exists) { throw Exception("Dokumen challenge tidak ditemukan!"); }
      final int duration = (challengeDoc.data() as Map<String, dynamic>)['duration'] ?? 1;

      // 2. Ambil data Partisipan (untuk cek progress pribadi)
      final participantDoc = await participantDocRef.get();
      if (!participantDoc.exists) { throw Exception("Kamu belum bergabung challenge ini!"); }
      
      final participantData = participantDoc.data() as Map<String, dynamic>;
      final int userProgress = participantData['progress'] ?? 0;

      // 3. Cek progress PRIBADI, bukan progress challenge
      if (userProgress < duration) {
        const int pointsToAdd = 10;
        final userDoc = await userDocRef.get();
        if (!userDoc.exists) { throw Exception("Dokumen user-mu tidak ditemukan."); }
        final userData = userDoc.data() as Map<String, dynamic>;

        final batch = FirebaseFirestore.instance.batch();
        
        // Update 1: Challenge (kita tidak lagi update progress challenge)
        // batch.update(challengeDocRef, {'progress': FieldValue.increment(1)}); // <-- HAPUS/KOMEN INI
        
        // Update 2: Poin global user
        batch.update(userDocRef, {'totalPoints': FieldValue.increment(pointsToAdd)});
        
        // Update 3: Poin dan Progress spesifik user
        batch.set(participantDocRef, {
          'score': FieldValue.increment(pointsToAdd),
          'progress': FieldValue.increment(1), // <-- 4. TAMBAHKAN UPDATE PROGRESS PRIBADI
          'userId': _user!.uid,
          'challengeId': widget.challengeId,
...
_checkRankAndNotify();
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text('Hore! Progres +1 dan kamu dapat +$pointsToAdd poin!'),
         ...
         if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text('Luar biasa! Kamu sudah menyelesaikan challenge ini.'),
...
@override
  ...
  Widget _buildDynamicButton() {
    ...
    return StreamBuilder<DocumentSnapshot>(
      stream: FirebaseFirestore.instance.collection('participants').doc(participantDocId).snapshots(),
      builder: (context, snapshot) {
        final bool hasJoined = snapshot.hasData && snapshot.data!.exists;
        if (hasJoined) {
          return ElevatedButton(
            onPressed: _isLoading ? null : _updateProgress,
            style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFF007AFF), padding: const EdgeInsets.symmetric(vertical: 16), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12))),
            child: _isLoading ? const SizedBox(height: 20, width: 20, child: CircularProgressIndicator(color: Colors.white, strokeWidth: 3)) : const Text('Tandai Selesai Hari Ini', style: TextStyle(color: Colors.white, fontSize: 16, fontWeight: FontWeight.bold)),
          );
        }
        return ElevatedButton(
          ...
          child: _isLoading ? const SizedBox(height: 20, width: 20, child: CircularProgressIndicator(color: Colors.white, strokeWidth: 3)) : const Text('Join Challenge', style: TextStyle(color: Colors.white, fontSize: 16, fontWeight: FontWeight.bold)),
        );
      },
    );
  }
  ...
  Widget _buildUserProgressRow({required int duration}) {
    ...
    return StreamBuilder<DocumentSnapshot>(
      ...
      builder: (context, snapshot) {
        ...
        return _buildInfoRow(icon: Icons.check_circle_outline, title: 'Progres Kamu', value: '$userProgress / $duration hari');
      },
    );
  }
  ...
  Widget _buildParticipantList({required int totalMembers}) {
    return StreamBuilder<QuerySnapshot>(
      stream: FirebaseFirestore.instance.collection('participants').where('challengeId', isEqualTo: widget.challengeId).limit(5).snapshots(),
      builder: (context, snapshot) {
        if (!snapshot.hasData || snapshot.data!.docs.isEmpty) {
          return _buildInfoRow(icon: Icons.people_outline, title: 'Total Anggota', value: '$totalMembers orang');
        }
        ...
        for (int i = 0; i < participants.length; i++) {
          ...
          avatarWidgets.add(Positioned(left: (i * 25.0), child: CircleAvatar(radius: 18, backgroundColor: const Color(0xFF1A1A1A), child: CircleAvatar(radius: 16, backgroundImage: NetworkImage(photoUrl), backgroundColor: Colors.grey.shade800))));
        }
        return Padding(
          padding: const EdgeInsets.symmetric(vertical: 12.0),
          child: Row(children: [
            ...
            const SizedBox(width: 16),
            Text('Anggota', style: TextStyle(color: Colors.grey.shade400, fontSize: 16)),
            const Spacer(),
            SizedBox(width: (participants.length * 25.0) + 10.0, height: 36, child: Stack(children: avatarWidgets)),
            if (totalMembers > 5) Padding(padding: const EdgeInsets.only(left: 8.0), child: Text('+${totalMembers - 5}', style: TextStyle(color: Colors.grey.shade400, fontSize: 14)))
          ]),
        );
      },
    );
